...
### to the power of 0.6 Transformation
Try log transformation makes it more normal. This will help to meet the multiple linear regression assumptions.
```{r}
ggplot(data, aes(x=popularity^0.6)) +
  geom_histogram(aes(y = stat(density)), bins = 30, fill = "white", col = "black") +
  stat_function(
    fun = dnorm, 
    args = list(mean = mean(data$popularity^0.6), sd = sd(data$popularity^0.6)), 
    lwd = 1, 
    col = 'red'
  ) +
  xlab("Popularity") + ylab("Density") +
  ggtitle(paste("Popularity", "Distribution")) +
  theme_bw()
summary(data$popularity^0.6)
```

```{r}
#produce normal qqplot to assess normality
qqnorm(data$popularity^0.6, pch = 1, frame = FALSE)
qqline(data$popularity^0.6, col = "red", lwd = 2)
#Skewness score
skewness(data$popularity^0.6)
kurtosis(data$popularity^0.6)
```

```{r}
#produce normal qqplot to assess normality
qqnorm(data$popularity^0.75, pch = 1, frame = FALSE)
qqline(data$popularity^0.75, col = "red", lwd = 2)
#Skewness score
skewness(data$popularity^0.75)
kurtosis(data$popularity^0.75)
```
...
check performance where the exponent is 0.6
